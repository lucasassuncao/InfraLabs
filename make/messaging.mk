...
up-kafka:
	...
	docker compose -f messaging/kafka/docker-compose.yml up -d
	@echo "$(GREEN)ok Kafka UI:   http://localhost:8081$(RESET)"
	@echo "$(GREEN)ok Kafka:      localhost:9092$(RESET)"
	@echo "$(GREEN)ok Zookeeper:  localhost:2181$(RESET)"

up-rabbitmq:
	@echo "$(CYAN)>> Starting RabbitMQ...$(RESET)"
	docker compose -f messaging/rabbitmq/docker-compose.yml up -d
	@echo "$(GREEN)ok RabbitMQ:  http://localhost:15672  (admin/admin)$(RESET)"

up-nats:
	@echo "$(CYAN)>> Starting NATS...$(RESET)"
	docker compose -f messaging/nats/docker-compose.yml up -d
	@echo "$(GREEN)ok NATS client:     localhost:4222$(RESET)"
	@echo "$(GREEN)ok NATS monitoring: http://localhost:8222$(RESET)"
	@echo "$(GREEN)ok NATS metrics:    http://localhost:7777/metrics$(RESET)"
