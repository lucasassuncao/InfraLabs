...
up-lamp:
	...
	docker compose -f web/lamp/docker-compose.yml up -d
	@echo "$(GREEN)✔ Apache/PHP:  http://localhost:8090$(RESET)"
	@echo "$(GREEN)✔ phpMyAdmin:  http://localhost:5053$(RESET)"
	@echo "$(GREEN)✔ MySQL:       localhost:3308  (admin/admin)$(RESET)"

up-lemp:
	@echo "$(CYAN)▶ Starting LEMP stack...$(RESET)"
	docker compose -f web/lemp/docker-compose.yml up -d
	@echo "$(GREEN)✔ NGINX/PHP:   http://localhost:8092$(RESET)"
	@echo "$(GREEN)✔ phpMyAdmin:  http://localhost:5054$(RESET)"
	@echo "$(GREEN)✔ MySQL:       localhost:3309  (admin/admin)$(RESET)"

up-mailpit:
	@echo "$(CYAN)▶ Starting Mailpit...$(RESET)"
	docker compose -f web/mailpit/docker-compose.yml up -d
	@echo "$(GREEN)✔ Mailpit UI:  http://localhost:8025$(RESET)"
	@echo "$(GREEN)✔ SMTP:        localhost:1025$(RESET)"

up-hugo:
	@echo "$(CYAN)▶ Starting Hugo...$(RESET)"
	docker compose -f web/hugo/docker-compose.yml up -d
	@echo "$(GREEN)✔ Hugo dev server: http://localhost:1313  (live reload)$(RESET)"
